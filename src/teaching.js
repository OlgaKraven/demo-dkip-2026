...
(function(root){
 ...
 function guide(){
  ...
  <section><span class="section-code">01 · ПЕРЕД ЗАНЯТИЕМ</span><h3>Укажите преподавателя</h3><p>Откройте «Настройка перед занятием» в верхнем меню. Заполните ФИО, должность, подразделение, учебное заведение и ссылку на материалы. Нажмите «Сохранить настройки». Для переноса на другой компьютер скачайте данные и загрузите JSON в том же окне.</p></section>
  <section><span class="section-code">02 · РАЗБОР</span><h3>Выберите модуль и СУБД</h3><p>В режиме «Разбор» выберите M1–M5 и MySQL или PostgreSQL. «Как сделать пошагово» объясняет создание собственной работы; «Как посмотреть готовый пример» помогает изучить подготовленный проект. Код раскрывается нажатием на название файла.</p></section>
  <section><span class="section-code">03 · ПРЕЗЕНТАЦИЯ</span><h3>Показывайте по одному элементу</h3><p>Нажмите «Презентация разбора». После титула и материалов откроется карта шагов. Каждый шаг разделён на отдельные блоки объяснения, таблицы, изображения и код. Длинные таблицы разбиты на группы строк, код — на фрагменты с номерами строк. Полные файлы доступны по ссылкам.</p><p>Стрелки ← →, Page Up / Page Down и пробел листают слайды; Home / End переходят в начало и конец. При фокусе в поле ввода, на кнопке или ссылке клавиши сохраняют обычное действие. «Содержание» открывает список слайдов; Esc закрывает окно. Позиция сохраняется отдельно для каждого модуля, СУБД и вида разбора.</p></section>
  <section><span class="section-code">04 · ВНЕШНИЙ ВИД</span><h3>Тема и полный экран</h3><p>Кнопка с луной / солнцем меняет сайт и слайды. Выбор сохраняется после перезагрузки; при первом открытии используется тема устройства. «На весь экран» увеличивает область показа. Если браузер не разрешает полный экран, разверните окно вручную.</p></section>
  <section><span class="section-code">05 · ТРЕНИРОВКА</span><h3>Отрабатывайте задания</h3><p>В «Моей тренировке» выберите задание, скачайте материалы и включите таймер. Доступны пауза, подсказки и отметки самопроверки. В «Пробном экзамене» выберите свой вариант V01–V30: общий таймер рассчитан на 150 минут, паузы нет. Переключение разделов и перезагрузка не останавливают запущенный таймер. В разборе и презентации таймера нет.</p></section>
  <section><span class="section-code">06 · СОХРАНЕНИЕ</span><h3>Сохраняйте работу в своих программах</h3><p>Сайт хранит локальные настройки и отметки, но не отправляет решения преподавателю. SQL, C#, PDF и другие результаты сохраняйте в папку сдачи. Очистка данных браузера удаляет локальный прогресс. Скачайте протокол до новой попытки и экспортируйте профиль преподавателя перед переносом.</p></section>
  <section><span class="section-code">07 · C# И БАЗА</span><h3>Запустите пример</h3><p>Скачайте полный пример выбранной СУБД. На Windows откройте Polesie.csproj в Visual Studio с компонентом «Разработка классических приложений .NET» и .NET Framework 4.8. Создайте пустую базу, выполните SQL-файлы 01–03, измените ConnectionString в Db.cs, соберите проект и запустите приложение. Подробности — README внутри архива. Нативный запуск WinForms на macOS не поддерживается.</p></section>
  </div>`;closeButton();
 }
 ...
 function mount(input){
  ...
  const tools=document.querySelector('.top-tools');tools.prepend(iconButton(button('Как пользоваться',guide),'Как пользоваться',guideIcon));tools.append(iconButton(button('Настройка перед занятием',settings),'Настройка перед занятием',settingsIcon));
  iconButton(document.querySelector('#fullscreen'),'На весь экран',fullIcon);
  themeButton(document.querySelector('#theme'));
  const hero=document.querySelector('.hero');const mascot=document.createElement('div');mascot.className='rhino-mascot';mascot.innerHTML='<img src="mascot-scientist.png" width="1075" height="1463" alt="Носорог-учёный с книгами и логотипом на кармане халата">';hero.append(mascot);const block=document.createElement('div');block.id='instructor-profile';block.innerHTML=profileHtml();hero.append(block);
  if(ctx.mode==='learn'){
   const route=document.querySelector('.lesson-route-tabs');const launch=button('Презентация разбора',presentation);launch.classList.add('primary');launch.id='presentation-launch';route.append(launch);
  }
